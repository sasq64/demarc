        ...
        let context = {
            ...
            loop {
                ...
                match (result, robustness) {
                    ...
                    // 27.0.4 only retries on BadAttribute, but per the EGL spec a
                    // robust-access context that the driver can't provide fails
                    // with BadMatch (core) or BadConfig (EXT robustness). Mesa's
                    // crocus driver (Intel Gen7, e.g. HD 4000) hits exactly this,
                    // so without retrying on them wgpu can't create any GL context
                    // and Bevy falls back to software rendering. Retry on all
                    // three so we reach the plain (non-robust) context such
                    // drivers accept. Backport of upstream fix wgpu#9153, which
                    // shipped in wgpu >= 29.0.3 (i.e. Bevy >= 0.19); drop this
                    // patch once demarc moves off Bevy 0.18 / wgpu 27.
                    (
                        Err(khronos_egl::Error::BadAttribute
