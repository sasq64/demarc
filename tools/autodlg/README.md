...
| `--prefer <a,b,c>` | Select the option whose label contains `a`; if the dialog offers no such option, try `b`, then `c`. Works on radio buttons, checkboxes, combo boxes and list boxes, because dialogs offer resolution as all four. Typically the wanted mode and what to settle for, e.g. `--prefer 800x600,640x480`. Repeatable, and each occurrence is a chain of its own, so several unrelated options can be picked in one run. Chains are applied in the order given — demarc passes `--prefer 16:9` before the resolutions on a widescreen, since the aspect can change the mode list. |
| `--check <label>` | Tick the checkbox whose label contains `<label>`, if it isn't ticked — repeatable. demarc passes `--check Fullscreen`. Applied before `--prefer`, because a switch like this one often rebuilds the resolution list: *Finally Inside* offers twenty display modes ticked and five windowed sizes unticked. |
