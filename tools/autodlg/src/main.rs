...
fn main() {
    ...
    while !driven && start.elapsed() < Duration::from_secs_f64(args.timeout) {
        for top in top_levels() {
    ...
    // Outlive the demo, so whoever started this driver can treat its exit as
    // the demo's exit. Without a launch there is nothing to wait for.
    match demo {
        Some(handle) => {
